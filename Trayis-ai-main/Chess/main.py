...
from TrayisAi import findRandomMoves, findBestMove
...
MAX_FPS = float('inf')  # for animations
IMAGES = {}
...
def pawnPromotionPopup(screen, gs):
    ...
    while True:
        ...
        screen.fill(p.Color((237, 238, 209)))
...
def main():
    ...
    while running:
        ...
        if moveMade:
            if countMovesForDraw <=5:
                countMovesForDraw += 1
            if countMovesForDraw == 6:
...
def drawMoveLog(screen, gs, font):
    ...
    p.draw.rect(screen, p.Color((237, 238, 209)), moveLogRect)
    moveLog = gs.moveLog
    moveTexts = []
